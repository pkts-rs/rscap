...
impl Npcap {
            ...
            Ok(Self {
                container: Container::load("Packet.dll").map_err(|e| match e {
                    ...
                    dlopen2::Error::OpeningLibraryError(e) => {
                        NpcapError::new(NpcapErrorKind::DllNotFound, e)
                    }
                    dlopen2::Error::SymbolGettingError(e) => {
                        NpcapError::new(NpcapErrorKind::MissingDllSymbol, e)
                    }
                    dlopen2::Error::NullSymbol => NpcapError::new(
                        NpcapErrorKind::MissingDllSymbol,
                        "the value of a resolved dll symbol was null",
                    ),
                    dlopen2::Error::AddrNotMatchingDll(e) => {
                        NpcapError::new(NpcapErrorKind::DllNotFound, e)
                    }
                })?,
            })
    ...
    pub fn set_bpf(&self, adapter: &mut Adapter, program: &BpfProgram) -> bool {
        match unsafe {
            self.container
                .PacketSetBpf(adapter, program as *const BpfProgram)
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn get_stats(&self, adapter: &mut Adapter, stats: &mut BpfStat) -> bool {
        match unsafe {
            self.container
                .PacketGetStats(adapter, stats as *mut BpfStat)
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn get_stats_ex(&self, adapter: &mut Adapter, stats: &mut BpfStat) -> bool {
        match unsafe {
            self.container
                .PacketGetStatsEx(adapter, stats as *mut BpfStat)
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn is_loopback_adapter(&self, adapter_name: &CStr) -> bool {
        match unsafe {
            self.container
                .PacketIsLoopbackAdapter(adapter_name.as_ptr())
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn send_packet(&self, adapter: &mut Adapter, packet: &mut Packet) -> bool {
        match unsafe {
            self.container
                .PacketSendPacket(adapter, packet as *mut Packet, 1)
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn send_packets(&self, adapter: &mut Adapter, packets: &mut [Packet]) -> libc::c_int {
        ...
        unsafe {
            self.container
                .PacketSendPackets(adapter, packets_ptr, packets_len, 1)
        }
    }
    ...
    pub fn init_packet(&self, packet: &mut Packet, buffer: NonNull<u8>, buflen: usize) {
        ...
        unsafe {
            self.container
                .PacketInitPacket(packet, buffer_ptr, buffer_len)
        }
    }
    ...
    pub fn receive_packet(&self, adapter: &mut Adapter, packet: &mut Packet) -> bool {
        match unsafe {
            self.container
                .PacketReceivePacket(adapter, packet as *mut Packet, 1)
        } {
            0 => false,
            _ => true,
        }
    ...
    pub fn get_net_info_ex(
        ...
        match unsafe {
            self.container
                .PacketGetNetInfoEx(adapter, addrs_ptr, entries_ptr)
        } {
            0 => false,
            _ => true,
        }
